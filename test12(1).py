...
def getGoodLocations():
    for tweet in good_tweets:
        if(tweet_dict[tweet.text].polarity > 0.5):
            if(tweet.user.location not in good_locations):
                good_locations.append(tweet.user.location)


def getBadLocations():
    for tweet in bad_tweets:
        if(tweet_dict[tweet.text].polarity < -0.5):
            if(tweet.user.location not in bad_locations):
                bad_locations.append(tweet.user.location)
...
getGoodLocations()
getBadLocations()
print("Here are where are fans are from", good_locations)
print("Here are where some of our haters are from",  bad_locations)
